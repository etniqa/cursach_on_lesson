<#import "/spring.ftl" as spring>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Title</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
</head>
<body>
<h3>organization list</h3>
<table class="table">
    <tr>
        <th>ID</th>
        <th>name</th>
        <th>description</th>
        <th>creationDate</th>
        <th>dateModified</th>
        <th>headOrganization</th>

        <th>delete</th>
        <th>edit</th>
    </tr>
    <#list organizations as organization>
        <tr>
            <td>${(organization.id)!"null"}</td>
            <td>${(organization.name)!"null"}</td>
            <td>${(organization.description)!"null"}</td>
            <td>${(organization.creationDate)!"null"}</td>
            <td>${(organization.dateModified)!"null"}</td>

            <td>${(organization.headOrganization.name)!"null"}</td>

            <td><a href='/web/organization/delete/${organization.id}'><button>delete</button></a></td>
            <td><a href='/web/organization/edit/${organization.id}'><button>edit</button></a></td>
        </tr>
    </#list>
</table>
<a href='/web/organization/add'><button>create</button></a>
</body>
</html>